"""Camera driver abstraction supporting USB (OpenCV) and Intel RealSense."""
...
class CameraDriver:
    ...
    def read(self) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        """Return (rgb_frame, depth_frame). Depth may be None for USB cameras."""
        if self.camera_type == "usb":
            if getattr(self, "_fallback_synthetic", False):
